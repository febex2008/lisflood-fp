...
)
{
	...
	for (int j=global_j+1; j<=max_j; j+=blockDim.y*gridDim.y)
	{
		for (int i=global_i; i<=max_i; i+=(blockDim.x-1)*gridDim.x)
		{
			...
			if (i <= cuda::geometry.xsz && j <= cuda::geometry.ysz)
			{
				if (i == 0 && cuda::geometry.xsz > 1
						&& cuda::boundaries.BC_type[Boundary::index_w(i, j)] == TRANSMISSIVE9)
					F[threadIdx.y][threadIdx.x] = F[threadIdx.y][threadIdx.x+1];
				else if (i == cuda::geometry.xsz && cuda::geometry.xsz > 1
						&& cuda::boundaries.BC_type[Boundary::index_e(i, j)] == TRANSMISSIVE9)
					F[threadIdx.y][threadIdx.x] = F[threadIdx.y][threadIdx.x-1];
			}
...
)
{
	...
	for (int j=global_j; j<=max_j; j+=(blockDim.y-1)*gridDim.y)
	{
		...
		for (int i=global_i+1; i<=max_i; i+=blockDim.x*gridDim.x)
		{
			...
			if (i <= cuda::geometry.xsz && j <= cuda::geometry.ysz)
			{
				if (j == 0 && cuda::geometry.ysz > 1
						&& cuda::boundaries.BC_type[Boundary::index_n(i, j)] == TRANSMISSIVE9)
					F[threadIdx.y][threadIdx.x] = F[threadIdx.y+1][threadIdx.x];
				else if (j == cuda::geometry.ysz && cuda::geometry.ysz > 1
						&& cuda::boundaries.BC_type[Boundary::index_s(i, j)] == TRANSMISSIVE9)
					F[threadIdx.y][threadIdx.x] = F[threadIdx.y-1][threadIdx.x];
			}
